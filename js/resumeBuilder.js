var bio = {
  ...
  welcomeMessage:
    "Thanks for viewing my resum&#233. My recent projects are linked below. Please call or email if you have any questions.",
  biopic: "images/resume-img.jpg",
  skills: ["JavaScript", "React", "TypeScript", "Collaboration"],
  display: function () {
    var formattedRole = HTMLheaderRole.replace("%data%", bio.role);
    var formattedName = HTMLheaderName.replace("%data%", bio.name);
    $("#header").prepend(formattedName, formattedRole);
    var formattedBioPic = HTMLbioPic.replace("%data%", bio.biopic);
    var formattedWelcomeMessage = HTMLwelcomeMsgr.replace(
      "%data%",
      bio.welcomeMessage
    );
...
var projects = {
  projects: [
    ...
    //   dates: "2018",
    //   description:
    //     "I used Google Maps API to build a responsive and interactive app for exploring Dallas, TX. Please note, this is my current project, so I'm still building certain features.",
    //   url: "Maps.API.Project/index.html",
    //   images: ["images/dallas-skyline.jpg"],
    // },
    {
      title: "Get To Da Wataa! A Frogger Imitation Game",
      dates: "2017",
      description:
        "I used a canvas and learned alot about rendering for this project. I had a lot of fun building this one. Watch out for the bugs.",
      url: "getToTheWater/index.html",
      images: ["images/wataa.jpg"],
    },
    {
      title: "Simple Click Game - With Sphynxes",
      dates: "2017",
      description:
        "I used knockout JS to create a simple click game. Choose the name of a cat, and click the picture for more cats. This allowed me a lot of practice with functions. Silly names and sphynxes await you!",
      url: "knockout.cat-clicker/index.html",
      images: ["images/sphynx_5.jpg"],
    },
  ],
  ...
  display: function () {
    for (index = 0; index < projects.projects.length; index++) {
      ...
      ).replace("#", projects.projects[index].url);
      var formattedDates = HTMLprojectDates.replace(
        "%data%",
        projects.projects[index].dates
      );
      var formattedDescription = HTMLprojectDescription.replace(
        "%data%",
        projects.projects[index].description
      );
      ...
      for (var i = 0; i < projects.projects[index].images.length; i++) {
        var formattedImage = HTMLprojectImage.replace(
          "%data%",
          projects.projects[index].images[i]
        );
...
var education = {
  ...
  displayOnline: function () {
    for (var index = 0; index < education.onlineCourses.length; index++) {
      var formattedOnlineSchool = HTMLonlineTitle.replace(
        "%data%",
        education.onlineCourses[index].school
      ).replace("#", education.onlineCourses[index].url);
      var formattedOnlineTitle = HTMLonlineSchool.replace(
        "%data%",
        education.onlineCourses[index].title
      );
      var formattedOnlineSchoolTitle =
        formattedOnlineSchool + formattedOnlineTitle;
      var formattedOnlineDates = HTMLonlineDates.replace(
        "%data%",
        education.onlineCourses[index].dates
      );
      var formattedOnlineFocus = HTMLonlineFocus.replace(
        "%data%",
        education.onlineCourses[index].focus
      );
      $(".education-entry:last").append(
        HTMLonlineClasses,
        formattedOnlineSchoolTitle,
        formattedOnlineDates,
        formattedOnlineFocus
      );
    }
  },
  display: function () {
